import Foundation
 ...
 class HomeViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
     ...
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
         let cell = tableView.dequeueReusableCell(withIdentifier: "customCell", for: indexPath) as! TableViewCell  //.xibカスタムセル使用時
         let tweetDetaModel :TweetDetaModel = tweetDeta[tweetDeta.count - indexPath.row - 1]
         cell.nameLabel.text = tweetDetaModel.name
         cell.tweetLabel.text = tweetDetaModel.text
         cell.tweetDate.text = "\(dateFormat.string(from: tweetDetaModel.recordDate))"  //日付表示スタイルを変更
         return cell
     }
     //+ボタンを円形にする
